                        ...
                        <div class="d-flex w-100 justify-content-between  mb-2 py-2 border-bottom cart-item">
                            <div class="d-flex align-items-center col-8">
                                <form action="{{ route('cart.delete.id', $cart->id) }}" method="POST">
                                    @method('delete')
                                    @csrf
                                    <button class="btn btn-sm btn-outline-danger rem_item"
                                        onclick="return confirm('Are you sure?')">
                                        <span class="mr-2"><a data-id="{{ $cart->id }}"><i
                                                    class="fa fa-trash"></i></a></span>
                                    </button>
                                </form>

                                <img src="{{ asset('storage/product/' . $cart->product->images->first()->url) }}"
                                ...
                                <div>
                                    ...
                                    <p class="mb-1 mb-sm-1"><small><b>Price:</b> <span
                                                class="price">{{ number_format($cart->inventory->price) }}</span></small>
                                    </p>
                                    <div>
                                        <div class="input-group">
                                            ...
                                            <input type="number"
                                                class="form-control form-control-sm qty text-center cart-qty" placeholder=""
                                                aria-label="Example text with button addon" id="qty"
                            ...
                            <div class="col text-right align-items-center d-flex justify-content-end">
                                <h4><b class="total-amount"
                                        data-id="{{ $cart->id }}">{{ number_format($cart->total) }}</b></h4>
                            </div>
                        </div>
                    @endforeach
